...
export let snapToolActivated = false;
...
document.querySelector("#snap").addEventListener("click", function(e) {
    if(!snapToolActivated) {
        snapToolActivated = true;
        outputToolInfo("snap-tool");
    } else {
        snapToolActivated = false;
        outputToolInfo("snap-tool");
    }
})
...
function outputToolInfo(tool) {
    switch(tool) {
        ...
        case "standard-tool":
            ...
            break;
        case "snap-tool":
            toolInfo.innerHTML = `The snap tool snaps the cursor to the grid when glose to integer`;
            if(snapToolActivated) {
                toolInfo.innerHTML += `<br>STATUS: activated`
            } else {
                toolInfo.innerHTML += `<br>STATUS: disabled`
            }
            break;
    }
}